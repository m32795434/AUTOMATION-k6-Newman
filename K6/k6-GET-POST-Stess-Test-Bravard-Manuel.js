...
*/
export let options = {
  //TRYING (something like😅)a STRESS TEST 💪------------------------------------------------------<<<<<<<(CHANGES)
  stages: [
  ...
  thresholds: {
    ...
    //I CHECK AT LEAST A RATE>0.98 FOR THE SUCCSSESFUL LOGINS (ACCEPTANCE CRITERIA)------------------------------------------------------<<<<<<<(CHANGES)
    "check_200_onlogin_rate": ["rate>0.98"]
  },
  ext: {
    loadimpact: {
      // projectID: 3113635,
...
//NEW RATE EXCLUSIVELY FOR THE LOGIN------------------------------------------------------<<<<<<<(CHANGES)
let checkLogin200Rate = new Rate("check_200_onlogin_rate");
...
export default function () {
  ...
  group("Login", function () {
    ...
    const url = "http://test.k6.io"; //------------------------------------------------------<<<<<<<(CHANGES)
    res = http.post(`${url}/login.php`, { login: credentials.username, password: credentials.password, redir: '1', csrftoken: `${vars["csrftoken"]}` });

    // Validar la respuesta y registrar información ------------------------------------------------------<<<<<<<(CHANGES)
    /*
    if (res.status === 200) {
      console.log(`VU ${__VU}, Iteración ${__ITER}: Solicitud a ${url} exitosa.`);
    } else {
      console.error(`VU ${__VU}, Iteración ${__ITER}: Solicitud a ${url} falló. Código de estado:
      ${res.status}`);
    }*/
    // ADDING A RATE, INSTED TO USING THE console.log------------------------------------------------------<<<<<<<(CHANGES)
    let login200Check = check(res, {
      "HTTP status is 200": (r) => r.status === 200,
    });
    checkLogin200Rate.add(login200Check);
